...
export class GoalModel {
    constructor() {
        ...
        this.completeCoins = COMPLETE_COINS[0];
        this.crushQuantity = CRUSH_QUANTITY[0];
    ...
    setRandomGoalModel(cellTypeNum) {
        let randomNum = Math.floor(Math.random() * (COMPLETE_COINS.length - 1)) + 1;
        this.goalType = GOAL_TYPE_REVERSE[randomNum];
        this.completeCoins = COMPLETE_COINS[randomNum];
        this.crushQuantity = CRUSH_QUANTITY[randomNum];
        
        this.specifyColor = (this.goalType === GOAL_TYPE.SINGLE_COLOR) ? CELL_TYPE_REVERSE[Math.floor(Math.random() * cellTypeNum) + 1] : CELL_TYPE.EMPTY;
    }
    ...
    isConformGoal(model) {
        ...
        case GOAL_TYPE.SINGLE_COLOR:
            return model.type === this.specifyColor;
        case GOAL_TYPE.STRAIGHT:
            return model.status === CELL_STATUS.COLUMN || model.status === CELL_STATUS.LINE;
        case GOAL_TYPE.WRAP:
            return model.status === CELL_STATUS.WRAP;
        case GOAL_TYPE.BIRD:
            return model.status === CELL_STATUS.BIRD;
        default:
            return false;
        }
    }
